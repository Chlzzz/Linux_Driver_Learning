...
static void m_timer_callback(strcut timer_list* t) {
    ...
    mod_timer(&m_timer, jiffies_64 + msecs_to_jiffies(dev1.counter));
}
...
long cdev_test_ioctl(struct file *file, unsigned int cmd, unsigned long arg){
    ...
    switch(cmd) {
        case TIMER_OPEN:
            printk("process TIMER_OPEN\n");
            add_timer(&m_timer);
            break;
        case TIMER_CLOSE
            printk("process TIMER_CLOSE\n");
            del_timer(&m_timer);
            break;
        case TIMER_SET:
            printk("process TIMER_SET\n");
            dev_pri -> counter = arg;
            m_timer.expires = jiffries_64 + msecs_to_jiffries(dev_pri -> counter);
            break;
        default:     
            break;
    }
